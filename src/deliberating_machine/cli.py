...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Deliberating-Machine thinks for you about the meanings.",
        epilog="Example:  deliberating-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | deliberating-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | deliberating-machine         #

    $ deliberating-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="deliberating-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"deliberating-machine: {utterance}.")
        syslog.closelog()
